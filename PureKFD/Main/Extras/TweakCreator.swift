    ...
    func finishPackage(_ install: Bool) {
        if !generatingpackage {
            ...
            if !install {
                ...
                let results = installPackage(pkg: Package(name: name, bundleID: bundleID, author: author, desc: "", longdesc: nil, accent: nil, screenshots: nil, banner: nil, previewbg: nil, url: nil, pkgtype: "purekfd", purekfd: nil, misaka: nil, picasso: nil), path: tempFileURL, appData: appData)
    ...
    var body: some View {
        Group {
            ...
            if tweak["type"] as? String == "replacing" {
                ...
                TextField("Origin Path", text: Binding<String>(
                    get: {
                        self.tweak["originPath"] as? String ?? ""
                    },
                    set: { newValue in
    ...
    var body: some View {
        Group {
            ...
            .sheet(isPresented: $filePickerPopOverTemp) {
                NavigationView {
                    FileBrowser(appData: appData, currentFullPath: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.path, pickerpath: $pickedFilePath, popover: $filePickerPopOverTemp)

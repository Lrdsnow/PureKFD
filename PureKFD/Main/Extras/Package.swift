                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Button(action: {
                            ...
                            if package.pkgtype == "misaka" && !isInstalled {
                                if let releases = package.misaka?.Releases {
    ...
    if readPackageInfo {
        ...
        do {
            ...
            if !FileManager.default.fileExists(atPath: extractedDir.appendingPathComponent(url.lastPathComponent.removingFileExtensions(2)).appendingPathComponent("Overwrite").path) {
                ...
                return (nil, Package(name: "Misaka Package", bundleID: "\(UUID())", author: "Unknown", desc: "Unknown", longdesc: nil, accent: nil, screenshots: nil, banner: nil, previewbg: nil, url: nil, pkgtype: "misaka", purekfd: nil, misaka: nil, picasso: nil))
            }
        }
    }

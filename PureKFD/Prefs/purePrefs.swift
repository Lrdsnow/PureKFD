...
struct FullPrefView: View {
    ...
    init(json: String, pkgpath: String) {
        ...
        self._theming = State(initialValue: checkForTheming(in: json) ?? [:])
    ...
    var body: some View {
            if let jsonData = json.data(using: .utf8),
               ...
               let config = jsonObject as? [String: Any] {
                List {
                    generateView(from: config).listRowBackground((theming["plainlist"] as? Bool ?? false) ? Color.clear : Color(uiColor: .systemFill)).listRowSeparator((theming["plainlist"] as? Bool ?? false) ? .hidden : .visible)
                }.bgImage(appData).plainList(theming["plainlist"] as? Bool ?? false)

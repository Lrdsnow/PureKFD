...
class SettingsCustomizerManager {
    ...
    public static func changeLanguageLabels(restoring: Bool) throws {
        ...
        if FileManager.default.fileExists(atPath: filePath) {
            ...
            if restoring {
                try overwriteFile(at: filePath, with: plistData)
            } else {
                ...
                try overwriteFile(at: filePath, with: newData)
            }
            ...
            let plistData = try Data(contentsOf: backupURL)
            if restoring {
                try overwriteFile(at: filePath_iOS16, with: plistData)
            } else {
                var plist = try PropertyListSerialization.propertyList(from: plistData, options: [], format: nil) as! [String: Any]
                ...
                let newData = try UsefulFunctions.addEmptyData(matchingSize: plistData.count, to: plist)
                try overwriteFile(at: filePath_iOS16, with: newData)
            }
        } else {
    ...
    public static func apply() throws {
        ...
        if (!removesIcons && !removesLabels && !removesPreviews && !hasImage) && (!useSettingsFootnote || settingsFootnote == "") {
            ...
            if FileManager.default.fileExists(atPath: backupURL.path) {
                try overwriteFile(at: filePath, with: try Data(contentsOf: backupURL))
            }
            return
        }
